...
# Colab warm-start run for the Film-Abel KernelMix causal-hybrid ablation.
#
# Hybrid dynamic correction:
#   C_dyn = C_causalconv + 0.25 * C_direct_gated
#
# This keeps most of causal-convolution's reversal continuity while retaining a
# small diffusion-gated direct residual for local spatial flexibility.
#
# Colab usage:
...
RUN_ROOT="${RUN_ROOT:-./runs_film_abel_kernelmix_causalhybrid_256x64/${RUN_TAG}}"

ARCH="multiscale_green_grid_film_abel_kernelmix_causalhybrid"
EPOCHS="${EPOCHS:-2000}"
LR="${LR:-2e-6}"
...
MASTER_LOG="${LOG_DIR}/film_abel_kernelmix_causalhybrid_master.log"
TRAIN_LOG="${LOG_DIR}/film_abel_kernelmix_causalhybrid_training.log"
...
resolve_resume_ckpt() {
    ...
    local candidates=(
        ...
        "./runs_film_abel_kernelmix_causalconv_256x64/latest/checkpoints/pinn_thin_layer_catalytic_v9_6_multiscale_green_grid_film_abel_kernelmix_causalconv_best.pth"
        "./runs_film_abel_kernelmix_causal_256x64/latest/checkpoints/pinn_thin_layer_catalytic_v9_6_multiscale_green_grid_film_abel_kernelmix_causal_best.pth"
        "./runs_film_abel_kernelmix_256x64/20260704_090518/checkpoints/pinn_thin_layer_catalytic_v9_6_multiscale_green_grid_film_abel_kernelmix_best.pth"
    ...
    log "Set RESUME_CKPT=/path/to/film_abel_or_kernelmix_checkpoint.pth"
    return 1
}
...
log "=================================================="
log "PINN v9.6 Film-Abel KernelMix causal-hybrid warm-start run"
...
if [[ -f "$FDM_PKL" ]]; then
    ...
    "$PYTHON" -u compare_concentration_fields.py \
        ...
        --output-json "${FINAL_DIR}/film_abel_kernelmix_causalhybrid_concentration_metrics.json" \
        --output-npz "${FINAL_DIR}/film_abel_kernelmix_causalhybrid_concentration_fields.npz" \
        --output-figure "${FINAL_DIR}/film_abel_kernelmix_causalhybrid_residual_summary.png" \
        2>&1 | tee "${LOG_DIR}/film_abel_kernelmix_causalhybrid_final_compare.log"
else
