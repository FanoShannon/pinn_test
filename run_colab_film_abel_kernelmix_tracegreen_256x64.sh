...
# Colab run for the Film-Abel TraceGreen ablation.
#
# Main change versus FluxTrace:
#   FluxTrace used two traces and sewed them together:
#     G_flux[J](x,t) + h00 * (C_D_int^film - G_flux[J](0,t))
#
#   TraceGreen keeps the successful Film-Abel/interface-memory trace as the
#   single boundary source and propagates it into the external domain:
#     C_D(x,t) = HeatDirichlet[C_D_int^film](x,t)
#              + h01 * (0 - D_far)
#              + R_smooth(x,t)
#
# Colab usage:
#   %cd /content/gdrive/MyDrive/pinn_v96
#   !bash run_colab_film_abel_kernelmix_tracegreen_256x64.sh

WORK_DIR="${WORK_DIR:-/content/gdrive/MyDrive/pinn_v96}"
PYTHON="${PYTHON:-python}"
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT="${RUN_ROOT:-./runs_film_abel_kernelmix_tracegreen_256x64/${RUN_TAG}}"

ARCH="multiscale_green_grid_film_abel_kernelmix_tracegreen"
EPOCHS="${EPOCHS:-500}"
LR="${LR:-2e-6}"
...
MASTER_LOG="${LOG_DIR}/film_abel_kernelmix_tracegreen_master.log"
TRAIN_LOG="${LOG_DIR}/film_abel_kernelmix_tracegreen_training.log"
...
compare_one() {
    ...
    "$PYTHON" -u compare_concentration_fields.py \
        ...
        --output-json "${FINAL_DIR}/tracegreen_${label}_concentration_metrics.json" \
        --output-npz "${FINAL_DIR}/tracegreen_${label}_concentration_fields.npz" \
        --output-figure "${FINAL_DIR}/tracegreen_${label}_residual_summary.png" \
        2>&1 | tee "${LOG_DIR}/tracegreen_${label}_final_compare.log"
}

log "=================================================="
log "PINN v9.6 Film-Abel KernelMix TraceGreen run"
