...
# Colab run for the TraceGreen + matched Abel interface-memory ablation.
#
# Difference from run_colab_film_abel_kernelmix_tracegreen_256x64.sh:
#   - arch = multiscale_green_grid_film_abel_kernelmix_tracegreen_matchedabel
#   - external C_D_int -> C_D(x,t) still uses trace-preserving erfc quadrature
#   - interface J_rxn -> C_D_int uses singularity-matched Abel quadrature:
#       t - tau = t*u^2, plus exact piecewise-constant history weights
#
# Colab usage:
#   %cd /content/gdrive/MyDrive/pinn_v96
#   !bash run_colab_film_abel_kernelmix_tracegreen_matchedabel_256x64.sh

WORK_DIR="${WORK_DIR:-/content/gdrive/MyDrive/pinn_v96}"
PYTHON="${PYTHON:-python}"
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT="${RUN_ROOT:-./runs_film_abel_kernelmix_tracegreen_matchedabel_256x64/${RUN_TAG}}"
...
MASTER_LOG="${LOG_DIR}/tracegreen_matchedabel_master.log"
TRAIN_LOG="${LOG_DIR}/tracegreen_matchedabel_training.log"
...
resolve_resume_ckpt() {
    ...
    log "Set RESUME_CKPT=/path/to/tracegreen_or_intmemory_checkpoint.pth"
    return 1
}
...
compare_one() {
    ...
    "$PYTHON" -u compare_concentration_fields.py \
        ...
        --output-json "${FINAL_DIR}/tracegreen_matchedabel_${label}_concentration_metrics.json" \
        --output-npz "${FINAL_DIR}/tracegreen_matchedabel_${label}_concentration_fields.npz" \
        --output-figure "${FINAL_DIR}/tracegreen_matchedabel_${label}_residual_summary.png" \
        2>&1 | tee "${LOG_DIR}/tracegreen_matchedabel_${label}_final_compare.log"
}

log "=================================================="
log "PINN v9.6 TraceGreen matched-Abel run"
