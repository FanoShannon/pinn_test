...
# Colab warm-start run for the Film-Abel KernelMix causal ablation.
#
# Difference from run_colab_film_abel_kernelmix_256x64.sh:
#   - arch = multiscale_green_grid_film_abel_kernelmix_causal
#   - dynamic_correction is multiplied by a diffusion reachability gate
#       g(y,t) = exp(-y^2 / (4*D*t*alpha))
#   - the training loop logs/penalizes the dynamic correction jump around scan
#     reversal, so the external residual cannot act as an anti-causal free field.
#
# Colab usage:
...
RUN_ROOT="${RUN_ROOT:-./runs_film_abel_kernelmix_causal_256x64/${RUN_TAG}}"
...
MASTER_LOG="${LOG_DIR}/film_abel_kernelmix_causal_master.log"
TRAIN_LOG="${LOG_DIR}/film_abel_kernelmix_causal_training.log"
...
log "=================================================="
log "PINN v9.6 Film-Abel KernelMix causal warm-start run"
...
if [[ -f "$FDM_PKL" ]]; then
    ...
    "$PYTHON" -u compare_concentration_fields.py \
        ...
        --output-json "${FINAL_DIR}/film_abel_kernelmix_causal_concentration_metrics.json" \
        --output-npz "${FINAL_DIR}/film_abel_kernelmix_causal_concentration_fields.npz" \
        --output-figure "${FINAL_DIR}/film_abel_kernelmix_causal_residual_summary.png" \
        2>&1 | tee "${LOG_DIR}/film_abel_kernelmix_causal_final_compare.log"
else
