...
# Colab warm-start run for the Film-Abel KernelMix causal-hybrid interface-memory v2 ablation.
#
# Main change versus causal-hybrid-smooth:
#   C_D_prior += positive finite-memory boost
#   C_D_int = clamp(C_D_prior + DeltaC_D)
# rather than using the old logit/sigmoid residual as the main interface fix.
#
# Colab usage:
#   %cd /content/gdrive/MyDrive/pinn_v96
#   !bash run_colab_film_abel_kernelmix_causalhybrid_intmemory_256x64.sh

WORK_DIR="${WORK_DIR:-/content/gdrive/MyDrive/pinn_v96}"
PYTHON="${PYTHON:-python}"
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT="${RUN_ROOT:-./runs_film_abel_kernelmix_causalhybrid_intmemory_256x64/${RUN_TAG}}"

ARCH="multiscale_green_grid_film_abel_kernelmix_causalhybrid_intmemory"
EPOCHS="${EPOCHS:-500}"
LR="${LR:-2e-6}"
...
MASTER_LOG="${LOG_DIR}/film_abel_kernelmix_causalhybrid_intmemory_master.log"
TRAIN_LOG="${LOG_DIR}/film_abel_kernelmix_causalhybrid_intmemory_training.log"
...
resolve_resume_ckpt() {
    ...
    local latest_smooth
    latest_smooth="$(latest_checkpoint_in "./runs_film_abel_kernelmix_causalhybrid_smooth_256x64" "pinn_thin_layer_catalytic_v9_6_multiscale_green_grid_film_abel_kernelmix_causalhybrid_smooth_best.pth")"
    if [[ -n "$latest_smooth" && -f "$latest_smooth" ]]; then
        echo "$latest_smooth"
        return 0
    fi
...
log "=================================================="
log "PINN v9.6 Film-Abel KernelMix causal-hybrid interface-memory v2 run"
...
if [[ -f "$FDM_PKL" ]]; then
    ...
    "$PYTHON" -u compare_concentration_fields.py \
        ...
        --output-json "${FINAL_DIR}/film_abel_kernelmix_causalhybrid_intmemory_concentration_metrics.json" \
        --output-npz "${FINAL_DIR}/film_abel_kernelmix_causalhybrid_intmemory_concentration_fields.npz" \
        --output-figure "${FINAL_DIR}/film_abel_kernelmix_causalhybrid_intmemory_residual_summary.png" \
        2>&1 | tee "${LOG_DIR}/film_abel_kernelmix_causalhybrid_intmemory_final_compare.log"
else
