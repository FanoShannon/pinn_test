...
# Colab run for the TraceGreen + mixed Abel interface-memory ablation.
#
# Key idea:
#   A_mix[J] = (1 - lambda) * A_midpoint[J] + lambda * A_matched[J]
# with lambda learned from a near-zero initialization.  Watch
# `abel_mix_lambda` in the progress log to see whether training wants the
# singularity-matched Abel contribution.
#
# Colab usage:
#   %cd /content/gdrive/MyDrive/pinn_v96
#   !bash run_colab_film_abel_kernelmix_tracegreen_mixedabel_256x64.sh
#
...
RUN_ROOT="${RUN_ROOT:-./runs_film_abel_kernelmix_tracegreen_mixedabel_256x64/${RUN_TAG}}"

ARCH="multiscale_green_grid_film_abel_kernelmix_tracegreen_mixedabel"
EPOCHS="${EPOCHS:-500}"
LR="${LR:-2e-6}"
...
MASTER_LOG="${LOG_DIR}/tracegreen_mixedabel_master.log"
TRAIN_LOG="${LOG_DIR}/tracegreen_mixedabel_training.log"
...
resolve_resume_ckpt() {
    ...
    local patterns=(
        ...
        "./runs_film_abel_256x64_two_stage pinn_thin_layer_catalytic_v9_6_multiscale_green_grid_dynamic.pth"
    ...
    log "Set RESUME_CKPT=/path/to/dynamic_or_tracegreen_checkpoint.pth"
    return 1
}
...
compare_one() {
    ...
    "$PYTHON" -u compare_concentration_fields.py \
        ...
        --output-json "${FINAL_DIR}/tracegreen_mixedabel_${label}_concentration_metrics.json" \
        --output-npz "${FINAL_DIR}/tracegreen_mixedabel_${label}_concentration_fields.npz" \
        --output-figure "${FINAL_DIR}/tracegreen_mixedabel_${label}_residual_summary.png" \
        2>&1 | tee "${LOG_DIR}/tracegreen_mixedabel_${label}_final_compare.log"
}

log "=================================================="
log "PINN v9.6 TraceGreen mixed-Abel run"
