...
# Colab warm-start run for the Film-Abel + EMA-memory ablation.
#
# This script starts from an existing multiscale_green_grid_film_abel best
# checkpoint and trains the new multiscale_green_grid_film_abel_ema branch:
#   1. Film-Abel interface C_D_int gets a zero-initialized EMA anti-drift head.
#   2. The external C_C/C_D field uses the memory/multi-Green correction path.
#
# Colab usage:
#   %cd /content/gdrive/MyDrive/pinn_v96
#   !bash run_colab_film_abel_ema_256x64.sh
#
# Common overrides:
...
#   EPOCHS=6000 LR=3e-6 FDM_COMPARE_EVERY=500 \
#   bash run_colab_film_abel_ema_256x64.sh

WORK_DIR="${WORK_DIR:-/content/gdrive/MyDrive/pinn_v96}"
PYTHON="${PYTHON:-python}"
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT="${RUN_ROOT:-./runs_film_abel_ema_256x64/${RUN_TAG}}"

ARCH="multiscale_green_grid_film_abel_ema"
EPOCHS="${EPOCHS:-6000}"
LR="${LR:-3e-6}"
...
MASTER_LOG="${LOG_DIR}/film_abel_ema_master.log"
TRAIN_LOG="${LOG_DIR}/film_abel_ema_training.log"
...
log "=================================================="
log "PINN v9.6 Film-Abel EMA-memory warm-start run"
...
if [[ -f "$FDM_PKL" ]]; then
    ...
    "$PYTHON" -u compare_concentration_fields.py \
        ...
        --output-json "${FINAL_DIR}/film_abel_ema_concentration_metrics.json" \
        --output-npz "${FINAL_DIR}/film_abel_ema_concentration_fields.npz" \
        --output-figure "${FINAL_DIR}/film_abel_ema_residual_summary.png" \
        2>&1 | tee "${LOG_DIR}/film_abel_ema_final_compare.log"
else
