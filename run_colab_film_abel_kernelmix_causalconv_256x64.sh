...
# Colab warm-start run for the Film-Abel KernelMix causal-convolution ablation.
#
# Difference from the causal-gate ablation:
#   - arch = multiscale_green_grid_film_abel_kernelmix_causalconv
#   - dynamic correction is no longer a direct field NN(x,t,...)
#   - a small source network learns S_corr(t), then the external field receives
#     int_0^t S_corr(tau) K(y,t-tau) dtau with endpoint subtraction
#
# Colab usage:
...
RUN_ROOT="${RUN_ROOT:-./runs_film_abel_kernelmix_causalconv_256x64/${RUN_TAG}}"
...
MASTER_LOG="${LOG_DIR}/film_abel_kernelmix_causalconv_master.log"
TRAIN_LOG="${LOG_DIR}/film_abel_kernelmix_causalconv_training.log"
...
log "=================================================="
log "PINN v9.6 Film-Abel KernelMix causal-convolution warm-start run"
...
if [[ -f "$FDM_PKL" ]]; then
    ...
    "$PYTHON" -u compare_concentration_fields.py \
        ...
        --output-json "${FINAL_DIR}/film_abel_kernelmix_causalconv_concentration_metrics.json" \
        --output-npz "${FINAL_DIR}/film_abel_kernelmix_causalconv_concentration_fields.npz" \
        --output-figure "${FINAL_DIR}/film_abel_kernelmix_causalconv_residual_summary.png" \
        2>&1 | tee "${LOG_DIR}/film_abel_kernelmix_causalconv_final_compare.log"
else
